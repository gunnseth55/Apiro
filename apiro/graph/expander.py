...
class NodeExpander:
    ...
    def expand(self, node: Node, graph) -> list[Node]:
        ...
        """
        # Guard: skip poison nodes generated by a previous failed LLM call
        # so they don't cascade into repeated 5-minute timeout stalls.
        if node.claim.startswith("[Expansion failed"):
            logger.warning(f"[NodeExpander] Skipping poison node: '{node.claim[:60]}'")
            return []
